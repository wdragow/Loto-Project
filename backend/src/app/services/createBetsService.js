...
export default function createBets(
  ...
  if (qtd_of_numbers_to_create > max) {
    return {
      error: `Quantidade de números excedida! Deve ser de no mínimo ${min} e máximo ${max}.`,
    };
  } else if (qtd_of_numbers_to_create < min) {
    return {
      error: `Quantidade de números muito baixa! Deve ser de no mínimo ${min} e máximo ${max}.`,
    };
  } else {
    let array_of_bets = [];
